...
/// Attach this node to your scene and assign two RigidBody2D nodes.
...
void MagneticConstraint::_bind_methods() {
    ...
    ClassDB::bind_method(D_METHOD("set_max_distance", "max_distance"), &MagneticConstraint::set_max_distance);
    ClassDB::bind_method(D_METHOD("get_max_distance"), &MagneticConstraint::get_max_distance);
    ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "max_distance"), "set_max_distance", "get_max_distance");

    ClassDB::bind_method(D_METHOD("set_magnetic_force", "force"), &MagneticConstraint::set_magnetic_force);
    ClassDB::bind_method(D_METHOD("get_magnetic_force"), &MagneticConstraint::get_magnetic_force);
    ADD_PROPERTY(PropertyInfo(Variant::FLOAT, "magnetic_force"), "set_magnetic_force", "get_magnetic_force");

    ClassDB::bind_method(D_METHOD("set_polarity", "polarity"), &MagneticConstraint::set_polarity);
    ClassDB::bind_method(D_METHOD("get_polarity"), &MagneticConstraint::get_polarity);
    ...
    ClassDB::bind_method(D_METHOD("toggle_polarity"), &MagneticConstraint::toggle_polarity);
}

MagneticConstraint::MagneticConstraint() {
...
void MagneticConstraint::set_max_distance(float p_max_distance) { max_distance = p_max_distance; }
float MagneticConstraint::get_max_distance() const { return max_distance; }

void MagneticConstraint::set_magnetic_force(float p_force) { magnetic_force = p_force; }
float MagneticConstraint::get_magnetic_force() const { return magnetic_force; }

void MagneticConstraint::set_polarity(int p_polarity) { polarity = (p_polarity >= 0) ? 1 : -1; }
int MagneticConstraint::get_polarity() const { return polarity; }

void MagneticConstraint::toggle_polarity() {
    polarity = -polarity;
...
// Enable input processing so that _input() is called.
    void MagneticConstraint::_ready() {
        ...
        Node* found_node_a = get_parent()->find_child("RigidBody2D", true, false);
        Node* found_node_b = get_parent()->find_child("RigidBody2D2", true, false);

        if (found_node_a) {
            // Attempt to cast to RigidBody3D.
            body_a = Object::cast_to<RigidBody2D>(found_node_a);
            if (!body_a) {
                UtilityFunctions::print("Found node 'RigidBody2D' in the scene tree.");
            }
        } else {
            UtilityFunctions::print("Could not find node 'RigidBody2D' in the scene tree.");
        }

        if (found_node_b) {
            body_b = Object::cast_to<RigidBody2D>(found_node_b);
            if (!body_b) {
                UtilityFunctions::print("Found node 'RigidBody2D2' in the scene tree.");
            }
        } else {
            UtilityFunctions::print("Could not find node 'RigidBody2D2' in the scene tree.");
        }
    }

    // Check for input events. When the space bar is pressed, toggle the magnetic polarity.
    void MagneticConstraint::_input(const Ref<InputEvent> &p_event) {
...
void MagneticConstraint::_physics_process(double delta) {
    ...
    Vector2 pos_a = body_a->get_global_transform().get_origin();
    Vector2 pos_b = body_b->get_global_transform().get_origin();
    Vector2 dir = pos_b - pos_a;
    ...
    Vector2 force_vector = dir.normalized() * (force_magnitude * polarity);
