...
@onready var toggle_button2 = $VBoxContainer/HBoxContainer3/VertexDisplacementOnButton
@onready var toggle_button3 = $VBoxContainer/HBoxContainer4/HeightMapScaleHSlider
@onready var toggle_button4 = $VBoxContainer/HBoxContainer5/VertexDisplayScaleHSlider
...
func _ready() -> void:
	...
	toggle_button2.toggled.connect(Callable(self, "_on_vertex_displacement_on"))
	toggle_button3.value_changed.connect(Callable(self, "_on_height_map_scale_changed"))
	toggle_button4.value_changed.connect(Callable(self, "_on_vertex_shader_scale_changed"))
...
func _on_vertex_displacement_on(toggled_on: bool) -> void:
	root_script.vertex_displacement_on = toggled_on

# This function toggles the height map scale variable in the root script.
func _on_height_map_scale_changed(value_changed: float) -> void:
	root_script.height_map_scale_changed = value_changed

# This function toggles the vertex shader scale variable in the root script.
func _on_vertex_shader_scale_changed(value_changed: float) -> void:
	root_script.vertex_shader_scale_changed = value_changed
